...
# Parámetros
# ========================================
file = "solap(pMC)" # Nombre del fichero de datos (sin extensión)
...
# ========================================
for x in range(1,5): #Números del 1 al 4
    file = "solap(pMC)"
    file_out = file + str(x)
    file_in = file_out + ".dat"
    ...
    line, = ax.plot(xs, frames_data[0][1], "-", color="orange", label='Patrón 1', linewidth=1)
    lines.append(line)
    line, = ax.plot(xs, frames_data[0][2], "-", color="cornflowerblue", label='Patrón 2', linewidth=1)
    lines.append(line)
    line, = ax.plot(xs, frames_data[0][3], "-", color="mediumaquamarine", label='Patrón 3', linewidth=1)
    lines.append(line)
    line, = ax.plot(xs, frames_data[0][4], "-", color="orchid", label='Patrón 4', linewidth=1)
    lines.append(line)
    line, = ax.plot(xs, frames_data[0][5], "-", color="olive", label='Patrón 5', linewidth=1)
    lines.append(line)
    ax.legend()
